...
map('n', '<leader>yg', [[:let @+ = trim(system("git branch --show-current 2>/dev/null"))<cr>]], { desc = 'Yank git branch' }) -- main|dev|master

map('n', '<leader>yj', [[:let @+=luaeval('require"jsonpath".get()')<cr>]], { desc = 'Yank json path' }) -- .path.to.json.object

map('n', '<leader>yR', [[:let @a='' | %s/regex/\=setreg('A', submatch(0) . "\n")/n<c-f> ]], { desc = 'Yank regex capture group' })
